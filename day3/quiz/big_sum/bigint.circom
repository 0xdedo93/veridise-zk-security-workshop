...
template BigLessThan(n, k){
    ...
    for (var i = k - 2; i >= 0; i--) {
        ...
        if (i == k - 2) {
           ...
           eq_ands[i].b <== lt[k - 1].out; // *** FIXED: it was eq[k - 2].out;
           ors[i].a <== lt[k - 1].out;
           ors[i].b <== ands[i].out;
        } else {
...
template BigSubModP(n, k){
    ...
    // Ensure that sub.underflow is properly constrained
    signal underflow;
    underflow <== sub.underflow;  // Constrain underflow
    ...
    signal tmp[k];
    for (var i = 0; i < k; i++){
        tmp[i] <== (1 - underflow) * sub.out[i];  // Constrain sub.underflow
        out[i] <== tmp[i] + underflow * add.out[i];  // Constrain sub.underflow
    }
}