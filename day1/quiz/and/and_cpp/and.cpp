...
void binaryCheck_1_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void binaryCheck_1_run(uint ctx_index,Circom_CalcWit* ctx);
void And2_2_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void And2_2_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint sub_component_aux;
uint index_multiple_eq;
Fr_sub(&expaux[3],&signalValues[mySignalStart + 1],&circuitConstants[0]); // line circom 19
Fr_mul(&expaux[1],&signalValues[mySignalStart + 1],&expaux[3]); // line circom 19
Fr_eq(&expaux[0],&expaux[1],&circuitConstants[1]); // line circom 19
...
for (uint i = 0; i < 2; i++) {
std::string new_cmp_name = "binCheck"+ctx->generate_position_array(aux_dimensions, 1, i);
binaryCheck_1_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+i] = aux_cmp_num;
csoffset += 2 ;
aux_cmp_num += 1;
}
...
binaryCheck_1_run(mySubcomponents[cmp_index_ref],ctx);
